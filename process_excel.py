...
def add_minutes_from_time_zero(df, col_b):
    ...
    if not deliver_indices:
        # For baseline files without markers, calculate minutes based on time duration
        # Map the last 10 minutes to -20 to -10, earlier data to more negative values
        print(f"    Info: No 'Deliver Compound' marker found - treating as baseline file")
        if 'Time' in df.columns and len(df) > 0:
            try:
                ...
                if len(valid_times) > 0:
                    ...
                    total_duration_minutes = (max_time - min_time).total_seconds() / 60
                    
                    # Map times: last 10 minutes -> -20 to -10, earlier -> more negative
                    minutes_list = []
                    for idx in df.index:
                        if pd.notna(times.loc[idx]):
                            # Calculate minutes from the end
                            minutes_from_end = (max_time - times.loc[idx]).total_seconds() / 60
                            # Map: last 10 minutes (0-10 from end) -> -20 to -10
                            if minutes_from_end <= 10:
                                mapped_minute = -20 + (10 - minutes_from_end) / 10 * 10  # 0->-20, 10->-10
                            else:
                                # Earlier data: continue the negative scale
                                mapped_minute = -10 - (minutes_from_end - 10)
                            minutes_list.append(mapped_minute)
                        else:
                            minutes_list.append(None)
                    df['Minutes_from_Time0'] = minutes_list
                    print(f"    Mapped {len(valid_times)} time points: last 10 min -> -20 to -10")
                    return df
            except Exception as e:
                print(f"    Warning: Could not calculate minutes from time for baseline: {e}")
        
        # Fallback: if we can't calculate from time, use row-based indexing
        print(f"    Warning: Using row-based indexing as fallback")
...
def process_excel_file(input_path):
    ...
    for sheet_name in sheet_names:
        ...
        # Step 3: Add "Minutes from Time 0" column (needed for pre/post separation)
        print(f"  Adding 'Minutes_from_Time0' column (temporary, for averaging)...")
        df = add_minutes_from_time_zero(df, col_b)
        
        # Step 4: Calculate average values
        # Pre-treatment: average by minute (1 min intervals)
        # Post-treatment: average by 20 seconds
        ...
        if 'Time' in df.columns and 'Minutes_from_Time0' in df.columns:
            ...
            print(f"  Pre-treatment rows: {len(df_pre)}, Post-treatment rows: {len(df_post)}")
            ...
            # Average pre-treatment data by minute (1 min intervals)
            if len(df_pre) > 0 and numeric_cols:
                # Extract minute from time (HH:MM format) for pre-treatment data
                df_pre['Minute'] = df_pre['Time'].astype(str).str[:5]  # Get HH:MM
                ...
                df_pre_avg = df_pre_avg.rename(columns={'Minute': 'Time'})
                averaged_parts.append(df_pre_avg)
                print(f"  Pre-treatment: {len(df_pre)} rows -> {len(df_pre_avg)} rows (by minute)")
            
            # Average post-treatment data by 20 seconds
            if len(df_post) > 0 and numeric_cols:
                # Convert Time to datetime for precise 20-second grouping
                df_post['Time_dt'] = pd.to_datetime(df_post['Time'], format='%H:%M:%S', errors='coerce')
                # If that fails, try with Date
                if df_post['Time_dt'].isna().all() and 'Date' in df_post.columns:
                    df_post['DateTime'] = pd.to_datetime(df_post['Date'].astype(str) + ' ' + df_post['Time'].astype(str), errors='coerce')
                    df_post['Time_dt'] = df_post['DateTime'].dt.time.apply(lambda x: pd.Timestamp.combine(pd.Timestamp.today().date(), x) if pd.notna(x) else pd.NaT)
                
                # Round to nearest 20 seconds
                if df_post['Time_dt'].notna().any():
                    df_post['Time_20s'] = df_post['Time_dt'].dt.floor('20S').dt.strftime('%H:%M:%S')
                else:
                    # Fallback: use time string and round to nearest 20 seconds manually
                    def round_to_20s(time_str):
                        try:
                            parts = str(time_str).split(':')
                            if len(parts) >= 3:
                                h, m, s = int(parts[0]), int(parts[1]), int(parts[2])
                                total_sec = h * 3600 + m * 60 + s
                                rounded_sec = (total_sec // 20) * 20
                            ...
                            pass
                        return str(time_str)[:8]  # Return first 8 chars (HH:MM:SS)
                    df_post['Time_20s'] = df_post['Time'].apply(round_to_20s)
                
                # Group by Date and Time_20s
                group_cols = []
                if 'Date' in df_post.columns:
                    group_cols.append('Date')
                group_cols.append('Time_20s')
                ...
                df_post_avg = df_post_avg.rename(columns={'Time_20s': 'Time'})
                # Drop temporary columns
                df_post_avg = df_post_avg.drop(columns=['Time_dt', 'DateTime'], errors='ignore')
                averaged_parts.append(df_post_avg)
                print(f"  Post-treatment: {len(df_post)} rows -> {len(df_post_avg)} rows (by 20 seconds)")
            
            # Combine averaged pre and post data
            if averaged_parts:
                ...
                if len(df_markers) > 0:
                    ...
                    df_markers['Time'] = df_markers['Time'].astype(str).str[:8]  # Keep full time format
                ...
                for col in ['Minute', 'Time_20s', 'Time_dt', 'DateTime']:
                    if col in df.columns:
                        df = df.drop(columns=[col])
        
        # Recalculate Minutes_from_Time0 after averaging (since Time may have changed)
        print(f"  Recalculating 'Minutes_from_Time0' column after averaging...")
        df = add_minutes_from_time_zero(df, col_b)
...
def save_processed_file(processed_sheets, original_path, suffix="_processed"):
    ...
    output_path = os.path.join(os.path.dirname(original_path), f"{timestamp} {base_name}{suffix}.xlsx")
...
def main():
    ...
    # Filter to only process the specified files
    target_files = ["antidote baseline 301225.xlsx", "antidote 301225.xlsx"]
    xlsx_files = [f for f in xlsx_files if f in target_files]
    
    print(f"Found {len(xlsx_files)} Excel files to process: {xlsx_files}")
